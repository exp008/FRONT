...
const ParticipantsListPage = ({participants, setParticipants, isMock, setIsMock}:Props) => {
    ...
    const {participant_name} = useAppSelector((state:RootState) => state.participants)
    ...
    const fetchParticipants = async () => {
        try {
            ...
            const response = await fetch(`${env.VITE_API_URL}/api/participants/?participant_name=${participant_name.toLowerCase()}`)
